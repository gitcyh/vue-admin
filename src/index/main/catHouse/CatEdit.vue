<template>
    <div>
        <el-dialog v-model="visible" :show-close="false" draggable title="修改分类" append-to-body top="5vh">
            <template #header="{ close, titleId, titleClass }">
                <div>
                    <h6 :id="titleId" :class="titleClass">修改分类</h6>
...
const validatePId = (rule, value, callback) => {
    if (ruleForm.level === '1') {
        ...
        } else if (ruleForm.parentId === props.id) {
            callback(new Error('已存在该分类'));
        } else {
            callback();
        }
    }
...
const getLevel = function () {
    ...
    return ruleForm.deep
}

const updateCategory = function () {
    request.post(api.sysUpdateCategory, {
        id: ruleForm.id,
        name: ruleForm.name,
...
watch(visible, (newValue, oldValue) => {
    if (newValue) {
        request.get(api.sysGetCategory, {
            params: {
                id: props.id
            }
        }).then(res => {
            if (res.data.code == 200) {
                const { id, name, subTitle, level, parentId } = res.data.data.data;
                ruleForm.id = id;
                ruleForm.name = name;
                ruleForm.subTitle = subTitle;
                ruleForm.level = level == 1 ? '1' : '2';
                ruleForm.deep = level;
